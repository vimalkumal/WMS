...
class AgencyProfileController extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->model('Agency/AgencyProfileModel');
		$data['user_info'] = $this->AgencyProfileModel->get_user_details($user_id);
		$this->load->view('Agency/AgencyUserProfile',$data);
	}
	...
	public function changeUserDetails(){
		...
		$this->form_validation->set_rules('hUserEmail',		'Email Id', 	'valid_email');
		...
		if($this->form_validation->run()){
			...
			$this->load->model('Agency/AgencyProfileModel');
			$result=$this->AgencyProfileModel->editUserProfileDetails($paramArray);
		...
		redirect("Agency/AgencyProfileController");
	}
	...
	public function changeProfileImage()
	{
		...
		if($is_input_valid){
	        ...
	        if (!$this->upload->do_upload('fUserImage'))
	        {
	                ...
	                $process_task	= false;
	                // pr($error,1);
	                $valid_message.="User image should be .png .jpg .gif, size < 1mb, max width= 1024, max heighr=768";
	        }
	        else{
				// pr($user_image,1);
